...
def runSlowFuzz(build, seeds, runs):
	seed_scores = {}
	memory = {}
	for x in seeds.keys():
		output = run("""
				...
				-seed={1}
				""".format(runs, x), stdout=PIPE, stderr=PIPE, shell=True, universal_newlines=True)
		...
		for l in output:
			if 'rss' in l:
				rss = l.split('rss: ')[1].split('Mb')[1].split()
				#memory[x] = int(rss[0])
				break
	print(seed_scores)
	print(memory)
	return seed_scores, memory
...
if __name__ == '__main__':
	...
	if args.libfuzzer:
		...
		if args.path == 'clean':
			...
			optimal_seed, coverage_records = runOptimizationLibFuzzer(args.depth, args.path, args.time, seeds,
																	  range_dict, verbose=args.verbose)
			if args.verbose:
	...
	# slowfuzz build
		...
		seed_scores, _ = runSlowFuzz(args.build, optimal_seeds, args.time)
		...
		for _ in range(args.depth):
			...
			seed_scores, memory = runSlowFuzz(args.build, new_seeds, args.time)
			if args.verbose:
				print("SCORES: ", seed_scores)
				optimal_seed = sorted(seed_scores, key=seed_scores.get)[-1]
		seeds = {}
		seeds[optimal_seed] = 0
		converage_records = {}
		for _ in range(args.depth):
			seed_scores, memory = runSlowFuzz(args.build, seeds, args.explorationdepth)
			coverage_records = {**coverage_records, **seed_scores}
			new_seeds, range_dict = generateSeedsSlowFuzz(seed_scores, range_dict)
		optimal_seed = sorted(coverage_records, key=coverage_records.get)[-1]
		print(optimal_seed, coverage_records)
